...
public class Command {
    ...
    private static int setProperty(String key, boolean value, ServerCommandSource source) {
        if (CONFIG.set(key, value)) {
            LiteralText text = new LiteralText(String.format(Translation.translate("command.saplanting.property.set.success")
                , key, value));
            MutableText hover = new LiteralText(String.format(Translation.translate("config.saplanting.property.%s"), key));
            text.setStyle(Style.EMPTY.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, hover)));
            source.sendFeedback(text, false);
        } else {
            LiteralText text = new LiteralText(String.format(Translation.translate("command.saplanting.property.set.already")
                , key, value));
    ...
    private static int getProperty(String key, ServerCommandSource source) {
        LiteralText hover = new LiteralText(String.format(Translation.translate("config.saplanting.property.%s"), key));
        LiteralText text = new LiteralText(String.format(Translation.translate("command.saplanting.property.get")
    ...
    private static int updateLanguage(String name, ServerCommandSource source) {
        if (!CONFIG.set("language", name)) {
            MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.language.already"), name));
            source.sendError(text);
            return 0;
        }
        Translation.updateLanguage(name);
        MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.language.success"), name));
        source.sendFeedback(text, false);
        return 1;
    }

    private static int queryLanguage(ServerCommandSource source) {
        MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.language.query"), CONFIG.getAsString("language")));
    ...
    private static int load(ServerCommandSource source, boolean dedicated) {
        LiteralText text;
        LiteralText hover = new LiteralText(Translation.translate("command.saplanting.file.open"));
            ...
            MutableText reset = new LiteralText(Translation.translate("command.saplanting.reset"))
                .setStyle(CLICKABLE_COMMAND
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        new LiteralText(String.format(Translation.translate("command.saplanting.reset.hover"),
                            DEFAULT_CONFIG.getAsString(key))))));
            LiteralText hover = new LiteralText(String.format(Translation.translate("config.saplanting.property.%s"), key));
            MutableText property = new LiteralText(key).setStyle(CLICKABLE_COMMAND
                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, hover))
                .withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, String.format("/saplanting property %s ", key))));
    ...
    private static int addToBlackList(Item item, ServerCommandSource source) {
        ...
        if (!Saplanting.isPlantItem(item)) {
            MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.blackList.add.notPlant")
                , id));
            source.sendError(text);
            return 0;
        }
        ...
        if (CONFIG.addToBlackList(item)) {
            MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.blackList.add.success")
                , id));
        ...
        MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.blackList.add.inBlackList")
            , id));
        source.sendError(text);
        return 0;
    }
    ...
    private static int removeFromBlackList(Item item, ServerCommandSource source) {
        ...
        if (CONFIG.removeFromBlackList(item)) {
            MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.blackList.remove.success")
                , id));
            MutableText undo = new LiteralText(Translation.translate("command.saplanting.blackList.add.undo"))
                .setStyle(CLICKABLE_COMMAND
                    .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
        ...
        MutableText text = new LiteralText(String.format(Translation.translate("command.saplanting.blackList.remove.notInBlackList")
            , id));
        source.sendError(text);
        return 0;
    }
    ...
    private static int displayBlackList(int page, ServerCommandSource source) {
        ...
        source.sendFeedback(title, false);
        MutableText hover = new LiteralText(Translation.translate("command.saplanting.blackList.hover"));
